...
def F_3(x, dt):
    alpha = 1e-3
    y = dynamics.mypropagation_jerk(x, dt, 1, -1, alpha)
    return y[-1,:]
...
def H_station(y, t, stationPos0):
    ...
    obs = observe.get_observation_R(stationPos, y[0:3], 1)
    return obs
...
# 归一化
LU = constant.R_earth.value
TU = np.sqrt(LU*LU*LU/constant.GM_earth.value)
VU = LU/TU
AU = VU/TU
...
xs_ckf[:,0:3] = xs_ckf[:,0:3]*LU
xs_ckf[:,3:6] = xs_ckf[:,3:6]*VU
xs_ckf[:,6:9] = xs_ckf[:,6:9]*AU
xs_ckf[:,9:12] = xs_ckf[:,9:12]*AU/TU
...
# 保存数据
t=t.reshape(-1,1)
data = np.hstack((t,xs,r_observe,xs_ckf))
